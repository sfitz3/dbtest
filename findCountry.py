...
f2=open('no-country-subscriptions.csv','r')
#filename='no-country-subscriptions.csv'
fout=open('subscriptions-with-country.csv','w+')

#set the attributes for production or staging Aria
if isProduction:
    # Point to Production

    #ariaoutfile = 'ariaAccountDetails_Prod.txt'

    clientID= '5747004'
    auth= 'HBAETeMpu3Hn9nmQgD9dtnaMQtWqGfFG'

    UKclientID ='5748616'
    UKauth ='H87sM5nMYEY3ftHFKaSqbHYJksTnfY33'

    AUclientID = '5751418'
    AUauth = '49NT5WPwwvwcHTHNapKySHmShRRxpnNQ'
   ...
   # ariaObjectURL = 'https://secure.ariasystems.net/api/AriaQuery/objects.php'
    ...
    #ariaoutfile = 'ariaAccountDetails_stage.txt'
#              ^Remove XXX if ready to update - extra safety measure to prevent accidental update
# Set Update Flags: True will enable; False Disable.  Revert will repopulate old LHGUID using same existing file. Should always retain orginal to get back to original values.
...
ariaGetAcctUrl=ariaCoreURL + '?client_no=' + clientID + '&auth_key=' + auth + '&rest_call=get_acct_details_all&acct_no='
...
for line in reader(f2,delimiter=','):
    name,uuid,aria_us_id,aria_uk_id,aria_can_id,aria_au_id=line
    usurl = ''
    ukurl=''
    auurl=''
    country = 'Unknown'
    if aria_us_id != 'NULL':
        if len(aria_us_id) < 9:
            usurl = ariaGetAcctUrl+str(aria_us_id)+'&output_format=json'
            #print(usurl)
            ...
            fout.write(name+','+uuid+','+aria_us_id+','+aria_uk_id+','+aria_can_id+','+aria_au_id+','+country+'\n')

    elif aria_can_id != 'NULL':
        if len(aria_can_id) < 9:
            #print('UK ID: '+str(aria_uk_id))
            usurl = ariaGetAcctUrl+str(aria_can_id)+'&output_format=json'
            #print(ariaGetAcctUrl)
            response2 = requests.get(usurl)
            response_dict = response2.json()
            if response_dict['country'] != None:
    ...
    elif aria_uk_id != 'NULL':
        if len(aria_uk_id) < 9:
            #print('UK ID: '+str(aria_uk_id))
            ukurl = UKariaGetAcctUrl+str(aria_uk_id)+'&output_format=json'
            #print(UKariaGetAcctUrl)
            response2 = requests.get(ukurl)
            response_dict = response2.json()
            if response_dict['country'] != None:
                country = response_dict['country']
            fout.write(name+','+uuid+','+aria_us_id+','+aria_uk_id+','+aria_can_id+','+aria_au_id+','+country+'\n')

    elif aria_au_id != 'NULL':
        if len(aria_au_id) < 9:
            #print('AU ID: '+str(aria_au_id))
            auurl = AUariaGetAcctUrl+str(aria_au_id)+'&output_format=json'
            #print(AUariaGetAcctUrl)
            response2 = requests.get(auurl)
            response_dict = response2.json()
            if response_dict['country'] != None:
                country = response_dict['country']
            fout.write(name+','+uuid+','+aria_us_id+','+aria_uk_id+','+aria_can_id+','+aria_au_id+','+country+'\n')
