...
#Need to change this to a new API
...
if isProduction:
    ...
    ariaoutfile = 'ariaAccounts-paytype-Prod.txt'
...
def callAria (e_date1,e_date2):
    with open(ariaoutfile, 'a+') as outfile:
        ...
        ariaGetAcctUrl = ariaObjectURL + '?client_no=' + clientID + '&auth_key=' + auth + '&rest_call=get_account_details&query_string=where created >'+str(e_date1)+' and <'+str(e_date2)+'&limit=2000&output_format=json'
...
def datelooper(i,years,startyear):
    ...
    while m < (1 * 1):
        ...
        a_date2=str(a_year)+'-'+str(a_month)+'-'+str(a_day2)
